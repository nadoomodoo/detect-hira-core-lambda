// @ts-nocheck
import { parseToRgba } from "../internal/data-grid/color-parser";
import { CustomCell, GridCellKind, Item } from "../internal/data-grid/data-grid-types";
...
const renderer: CustomRenderer<SparklineCell> = {
  ...
  draw: (args, cell) => {
    ...
    values = values.map((x) =>
      Math.min(1, Math.max(0, (x - minY) / (maxY - minY))),
    );
    ...
    if (graphKind === 'bar') {
      ...
      if (
        ...
        ctx.fillText(
          displayValues[closest],
          drawX,
          rect.y + theme.cellVerticalPadding,
        );
        ctx.restore();
      }
    }
