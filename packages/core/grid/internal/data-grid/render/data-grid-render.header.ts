// @ts-nocheck
import { intersectRect, pointInRect } from '../../../common/math';
  ...
  walkColumns(
    ...
    (c, x, _y, clipX) => {
      ...
      const hover = noHover
        ? 0
        : hoverValues.find(
            (s) => s.item[0] === c.sourceIndex && s.item[1] === -1,
          )?.hoverAmount ?? 0;

      const hasSelectedCell =
        selection?.current !== undefined &&
        selection.current.cell[0] === c.sourceIndex;

      const bgFillStyle = selected
        ? theme.bgHeaderSelected
        : hasSelectedCell
...
export function drawGroups(
  ...
  walkGroups(
    ...
    (span, groupName, x, y, w, h) => {
      ...
      const groupTheme =
        group?.overrideTheme === undefined
          ? theme
          : mergeAndRealizeTheme(theme, group.overrideTheme);
      const isHovered =
        hRow === -2 && hCol !== undefined && hCol >= span[0] && hCol <= span[1];

      const fillColor = isHovered
        ? groupTheme.bgHeaderHovered
        : groupTheme.bgHeader;
      ...
      if (group !== undefined) {
        ...
        ctx.fillText(
          ...
          drawX + xPad,
          groupHeaderHeight / 2 +
            getMiddleCenterBias(ctx, theme.headerFontFull),
        );
...
export function computeHeaderLayout(
  ...
  return {
    ...
    indicatorIconBounds: flipHorizontal(
      indicatorIconBounds,
      mirrorPoint,
      isRTL,
    ),
...
function drawHeaderInner(
  ...
  if (c.rowMarker !== undefined && c.headerRowMarkerDisabled !== true) {
    ...
    const markerTheme =
      c.headerRowMarkerTheme !== undefined
        ? mergeAndRealizeTheme(theme, c.headerRowMarkerTheme)
        : theme;
    drawCheckbox(
      ctx,
      markerTheme,
  ...
  if (c.icon !== undefined && headerLayout.iconBounds !== undefined) {
    ...
    if (
      c.overlayIcon !== undefined &&
      headerLayout.iconOverlayBounds !== undefined
    ) {
      spriteManager.drawSprite(
        c.overlayIcon,
  ...
  if (shouldDrawMenu && headerLayout.menuBounds !== undefined) {
    ...
    if (
      ...
      const iconX =
        menuBounds.x + (menuBounds.width - theme.headerIconSize) / 2;
      const iconY =
        menuBounds.y + (menuBounds.height - theme.headerIconSize) / 2;
      spriteManager.drawSprite(
        c.menuIcon,
        'normal',
        ctx,
