...
function offsetColumnSize(
  ...
  return clamp(
    Math.round(width - (column.growOffset ?? 0)),
    Math.ceil(min),
    Math.floor(max),
  );
}
...
const DataGridDnd: React.FunctionComponent<DataGridDndProps> = (p) => {
  ...
  const canResize =
    (onColumnResize ?? onColumnResizeEnd ?? onColumnResizeStart) !== undefined;
      ...
      if (args.button === 0) {
        if (resizeCol !== undefined) {
          ...
          if (selectedColumns?.hasIndex(resizeCol) === true) {
            for (const c of selectedColumns?.toArray() ?? []) {
  ...
  const onDragStartImpl = React.useCallback<
    NonNullable<DataGridDndProps['onDragStart']>
  >(
    (args) => {
      onDragStart?.(args);
