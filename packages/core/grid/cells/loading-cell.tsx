...
export const loadingCellRenderer: InternalCellRenderer<LoadingCell> = {
  ...
  draw: (a) => {
    ...
    if (
      cell.skeletonWidthVariability !== undefined &&
      cell.skeletonWidthVariability > 0
    ) {
      width += Math.round(
        getRandomNumber(col, row) * cell.skeletonWidthVariability,
      );
    }
    ...
    const rectHeight =
      cell.skeletonHeight ??
      Math.min(18, rect.height - 2 * theme.cellVerticalPadding);
