...
export function decodeHTML(html: string): CopyBuffer | undefined {
  ...
  while (walkEl.length > 0) {
    ...
    if (el instanceof HTMLTableElement || el.nodeName === "TBODY") {
      walkEl.push(...[...el.children as any].reverse());
      ...
      const brs = clone.querySelectorAll("br") as any;
      for (const br of brs) {
        br.replaceWith("\n");
      }
      ...
      if (clone.querySelector("a") !== null) {
        ...
        current?.push({
          rawValue: [...rawValues as any].map(x => x.getAttribute("gdg-raw-value") ?? ""),
          formatted: [...rawValues as any].map(x => x.textContent ?? ""),
          format: "string-array",
        });
