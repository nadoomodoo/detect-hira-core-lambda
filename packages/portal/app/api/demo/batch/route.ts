...
import { API_BASE, endpointPath } from "@/lib/config";
...
/**
 * 배치 제출(범용) — API 종류 무관. slug 로 프로덕트를 찾고 apiKind 로 verb(extract|detect) 결정.
 * 로그인 회원 전용(본인 과금). 게이트웨이 내부 batch-async 로 위임.
 *  - 프로덕션(Cloud Tasks): 202 { jobId } → /api/demo/jobs/{id} 폴링.
 *  - 로컬(큐 미설정): 동기 처리 후 200 { jobId, results... }.
 */
export async function POST(req: Request) {
  ...
  if (!slug) return NextResponse.json({ error: "no_slug", message: "API slug 가 필요합니다." }, { status: 400 });
  ...
  const product = await prisma.product.findUnique({ where: { slug }, select: { apiKind: true, status: true } }).catch(() => null);
  if (!product || product.status === "DEPRECATED") return NextResponse.json({ error: "product_not_found", message: "해당 API를 찾을 수 없습니다." }, { status: 404 });

  const verb = endpointPath(product.apiKind); // extract | detect
  const templateId = typeof body?.templateId === "string" ? body.templateId : undefined;
  const model = typeof body?.model === "string" ? body.model : undefined;

  try {
    const resp = await fetch(`${GATEWAY}/internal/v1/${slug}/${verb}-batch-async`, {
      method: "POST",
