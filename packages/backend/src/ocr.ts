...
/**
 * Vertex AI Gemini 로 이미지에서 HIRA 약가코드(9자리)를 검출한다.
 *
 * - 환경변수는 vertex.txt 의 변수명을 우선 지원 (MODEL_NAME, VERTEX_PROJECT_ID 등).
 * - structured output (responseSchema) 로 JSON 배열을 강제하여
 *   각 항목이 { code, box: [y1,x1,y2,x2] } 형태가 되도록 한다.
 * - box 좌표는 0~1000 정규화 좌표 (Gemini 의 표준 bounding box 규약).
 */

const DEFAULT_MODEL = "gemini-3.1-flash-lite";

/**
 * 반복 루프(degeneration)일 때만 사용하는 에스컬레이션 모델.
 * Gemini 3.x 는 `global` 리전에서만 제공 — VERTEX_LOCATION=global 필요
 * (us-central1 등 리전 엔드포인트에서는 404).
 */
const DEFAULT_FALLBACK_MODEL = "gemini-3.5-flash";

/** 모델명별 클라이언트 캐시 (기본 모델 + 폴백 모델). */
const modelClients = new Map<string, ReturnType<VertexAI["getGenerativeModel"]>>();

/** 클라이언트 → 모델명 역참조 (사용량 집계 라벨용). */
const clientModelNames = new WeakMap<object, string>();
...
function resolveAuth(): {
  ...
  // 1) 서비스 계정 JSON 문자열 (vertex.txt 방식)
  ...
  if (credsJson) {
    try {
      ...
      throw new Error(
        "GOOGLE_APPLICATION_CREDENTIALS_JSON 파싱 실패. 올바른 JSON 인지 확인.",
      );
    }
  }

  // 2) Express mode API 키 (vertex.txt 방식)
  const apiKey = process.env.VERTEX_API_KEY;
  if (apiKey) return { apiKey };

  // 3) 표준 ADC (gcloud auth application-default login 또는 GOOGLE_APPLICATION_CREDENTIALS)
  return {};
}

/**
 * 리전에 맞는 API 엔드포인트.
 * 구 SDK 는 `{location}-aiplatform.googleapis.com` 을 조립하는데,
 * Gemini 3.x 가 제공되는 `global` 리전은 `aiplatform.googleapis.com` 을 써야 한다.
 */
function resolveApiEndpoint(location: string): string | undefined {
  return location === "global" ? "aiplatform.googleapis.com" : undefined;
}

/** 모델 클라이언트를 lazy 하게 생성/캐싱. modelName 생략 시 환경변수/기본 모델. */
function getModel(modelName?: string) {
  const model = modelName ?? process.env.MODEL_NAME ?? process.env.GEMINI_MODEL ?? DEFAULT_MODEL;
  const cached = modelClients.get(model);
  if (cached) return cached;

  // vertex.txt 변수명 우선, 표준 변수명 폴백
  ...
  if (!project) {
    throw new Error(
      "VERTEX_PROJECT_ID (또는 GOOGLE_CLOUD_PROJECT) 환경변수가 필요합니다. .env.example 참고.",
    );
  }
  ...
  const vertex = new VertexAI({
    project,
    location,
    apiEndpoint: resolveApiEndpoint(location),
    googleAuthOptions: auth.googleAuthOptions,
  });

  const client = vertex.getGenerativeModel({
    model,
    generationConfig: {
        ...
        items: {
          ...
          properties: {
            code: {
              ...
              description: "9자리 HIRA 약가코드 (숫자만)",
            },
            // Gemini 가 학습된 표준 bbox 규약의 필드명(box_2d)을 그대로 사용해야
            // 좌표 정확도가 유지된다 (임의 필드명은 모델별로 해석이 흔들림).
            box_2d: {
              type: FunctionDeclarationSchemaType.ARRAY,
              items: { type: FunctionDeclarationSchemaType.NUMBER },
              description:
                "bounding box [ymin, xmin, ymax, xmax], 각 값은 0~1000 정규화 좌표",
            },
          },
  ...
  modelClients.set(model, client);
  clientModelNames.set(client, model);
  return client;
}
...
export async function detectHiraCodes(
  ...
  const textPart: Part = { text: OCR_PROMPT };

  const request: GenerateContentRequest = {
    contents: [{ role: "user", parts: [filePart, textPart] }],
  };

  const text = await callModel(model, request, "ocr");
  ...
  for (let attempt = 0; isDegenerate(dets) && attempt < 2; attempt++) {
    const retry = mergeNearDuplicates(parseOcrResponse(await callModel(model, request, "ocr-retry")));
  ...
  if (isDegenerate(dets)) {
    try {
      const fallback = getModel(process.env.OCR_FALLBACK_MODEL ?? DEFAULT_FALLBACK_MODEL);
      const fbDets = mergeNearDuplicates(parseOcrResponse(await callModel(fallback, request, "ocr-fallback")));
...
async function callModel(
  client: ReturnType<VertexAI["getGenerativeModel"]>,
  request: GenerateContentRequest,
  purpose: string = "ocr",
): Promise<string> {
  const modelName = clientModelNames.get(client) ?? "unknown";
  const usage = usageBucket(`${purpose}(${modelName})`);
...
/** GenerateContentResponse 에서 텍스트 부분을 모두 이어붙여 반환. */
...
/** 회전 전용 모델 클라이언트 (경량 — enum 4값만 응답). */
let rotationClient: ReturnType<VertexAI["getGenerativeModel"]> | null = null;

function getRotationModel() {
  if (rotationClient) return rotationClient;

  const project = process.env.VERTEX_PROJECT_ID ?? process.env.GOOGLE_CLOUD_PROJECT;
  const location = process.env.VERTEX_LOCATION ?? process.env.GOOGLE_CLOUD_LOCATION ?? "us-central1";
  if (!project) {
    throw new Error(
      "VERTEX_PROJECT_ID (또는 GOOGLE_CLOUD_PROJECT) 환경변수가 필요합니다.",
    );
  }
  ...
  // 회전 감지는 가장 경량 모델로 충분 (같은 MODEL_NAME 사용).
  const model = process.env.MODEL_NAME ?? process.env.GEMINI_MODEL ?? DEFAULT_MODEL;

  rotationClient = vertex.getGenerativeModel({
    model,
    generationConfig: {
      temperature: 0,
      responseMimeType: "application/json",
      responseSchema: {
        ...
        properties: {
          rotation: {
            ...
            enum: ["0", "90", "180", "270"],
            description:
              "이미지가 정방향(위가 위) 기준으로 시계방향으로 몇 도 회전했는지. 0=정방향.",
          },
        },
        required: ["rotation"],
      },
  ...
  clientModelNames.set(rotationClient, model);
...
export async function detectRotation(
  ...
  const textPart: Part = { text: ROTATION_PROMPT };

  const request: GenerateContentRequest = {
    contents: [{ role: "user", parts: [filePart, textPart] }],
  };

  const text = await callModel(model, request, "rotation");
