...
import {
  RenderStateProvider,
  packColRowToNumber,
} from '../../common/render-state-provider';
...
import { pointInRect } from '../../common/math';
import {
...
import {
  ...
  drawHeader,
  computeHeaderLayout,
} from './render/data-grid-render.header';
...
export interface DataGridProps {
   ...
   * @group Events
   */
  readonly onHeaderIndicatorClick:
    | ((col: number, screenPosition: Rectangle) => void)
    | undefined;
  ...
  /** @group Drag and Drop */
  readonly onDragOverCell:
    | ((cell: Item, dataTransfer: DataTransfer | null) => void)
    | undefined;
  /** @group Drag and Drop */
  ...
  readonly getCellRenderer: <T extends InnerGridCell>(
    cell: T,
...
const DataGrid: React.ForwardRefRenderFunction<DataGridRef, DataGridProps> = (
  ...
  const cellXOffset = Math.max(
    freezeColumns,
    Math.min(columns.length - 1, cellXOffsetReal),
  );
  ...
  const [drawCursorOverride, setDrawCursorOverride] = React.useState<
    React.CSSProperties['cursor'] | undefined
  >();
  ...
  const enableFirefoxRescaling =
    (experimental?.enableFirefoxRescaling ?? false) && browserIsFirefox.value;
  const enableSafariRescaling =
    (experimental?.enableSafariRescaling ?? false) && browserIsSafari.value;
  React.useLayoutEffect(() => {
    if (
      window.devicePixelRatio === 1 ||
      (!enableFirefoxRescaling && !enableSafariRescaling)
    )
      return;
  ...
  const getMouseArgsForPosition = React.useCallback(
    (
      ...
      const isTouch =
        (ev !== undefined && !(ev instanceof MouseEvent)) ||
      ...
      if (
        ...
        if (horizontal === 0)
          innerHorizontal =
            col === -1
              ? OutOfBoundsRegionAxis.EndPadding
              : OutOfBoundsRegionAxis.Center;
        if (vertical === 0)
          innerVertical =
            row === undefined
              ? OutOfBoundsRegionAxis.EndPadding
        ...
        const isMaybeScrollbar =
          (x > width && x < width + getScrollBarWidth()) ||
          (y > height && y < height + getScrollBarWidth());
        ...
        if (fillHandle && selection.current !== undefined) {
          ...
          const fillHandleCellBounds = getBoundsForItem(
            canvas,
            fillHandleLocation[0],
            fillHandleLocation[1],
          );

          if (fillHandleCellBounds !== undefined) {
            const handleLogicalCenterX =
              fillHandleCellBounds.x + fillHandleCellBounds.width - 2;
            const handleLogicalCenterY =
              fillHandleCellBounds.y + fillHandleCellBounds.height - 2;
  ...
  const [bufferACtx, bufferBCtx] = React.useMemo(() => {
    ...
    return [
      a.getContext('2d', { alpha: false }),
      b.getContext('2d', { alpha: false }),
    ];
  }, []);
  ...
  const renderStateProvider = React.useMemo(
    () => new RenderStateProvider(),
    [],
  );

  const maxDPR =
    enableFirefoxRescaling && scrolling
      ? 1
      : enableSafariRescaling && scrolling
        ? 2
        : 5;
  const minimumCellWidth =
    experimental?.disableMinimumCellWidth === true ? 1 : 10;
  ...
  const draw = React.useCallback(() => {
    ...
    const current = {
      ...
      renderStrategy:
        ...
        (browserIsSafari.value ? 'double-buffer' : 'single-buffer'),
      getCellRenderer,
      minimumCellWidth,
      resizeIndicator,
    };
    ...
    if (
      ...
      (current.damage === undefined ||
        current.damage.has(hoverInfoRef?.current?.[0]))
    ) {
  ...
  if (
    ...
    editableBoolHovered =
      cell.kind === GridCellKind.Boolean && booleanCellIsEditable(cell);
  ...
  if (
    target !== null &&
    target !== undefined &&
    lastSetCursor.current !== style.cursor
  ) {
  ...
  const onMouseDownImpl = React.useCallback(
    (ev: MouseEvent | TouchEvent) => {
      ...
      if (
        canvas === null ||
        (ev.target !== canvas && ev.target !== eventTarget)
      )
        return;
      ...
      if (
        ...
        isOverHeaderElement(canvas, args.location[0], clientX, clientY) !==
          undefined
      ) {
        return;
  ...
  const onMouseUpImpl = React.useCallback(
    (ev: MouseEvent | TouchEvent) => {
      ...
      if (
        lastUpTimeValue !== 0 &&
        Date.now() - lastUpTimeValue < (args.isTouch ? 1000 : 500)
      ) {
        args = {
          ...args,
          isDoubleClick: true,
  ...
  const onMouseMoveImpl = React.useCallback(
    (ev: MouseEvent) => {
      ...
      const maybeSetHoveredInfo = (
        newVal: typeof hoveredItemInfo,
        ...
        setHoveredItemInfo((cv) => {
          ...
          if (
            cv?.[0][0] === newVal?.[0][0] &&
            cv?.[0][1] === newVal?.[0][1] &&
            ((cv?.[1][0] === newVal?.[1][0] && cv?.[1][1] === newVal?.[1][1]) ||
              !needPosition)
          ) {
            return cv;
          }
      ...
      if (!mouseEventArgsAreEqual(args, hoveredRef.current)) {
        ...
        if (args.kind === 'cell') {
          ...
          needsHoverPosition =
            rendererNeeds ?? toCheck.kind === GridCellKind.Custom;
  ...
  const onDragStartImpl = React.useCallback(
    (event: DragEvent) => {
      ...
      if (
        ...
        if (
          dragImage !== undefined &&
          dragImageX !== undefined &&
          dragImageY !== undefined
        ) {
  ...
  // Forward wheel events from canvas to scroller
  const onWheelImpl = React.useCallback(
                ...
                {effectiveCols.map((c) => {
                  ...
                  return (
                    <td
                      ...
                      aria-readonly={
                        isInnerOnlyCell(cellContent) ||
                        !isReadWriteCell(cellContent)
                      }
                      id={id}
                      data-testid={id}
                      onClick={() => {
