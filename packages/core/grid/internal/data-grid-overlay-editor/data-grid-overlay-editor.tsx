...
const DataGridOverlayEditor: React.FunctionComponent<
  DataGridOverlayEditorProps
> = (p) => {
  const {
    target,
    content,
  ...
  const [isValid, setIsValid] = React.useState(() => {
    ...
    return !(
      isEditableGridCell(content) &&
      validateCell?.(cell, content, lastValueRef.current) === false
    );
  });
  ...
  const customMotion = React.useRef<[-1 | 0 | 1, -1 | 0 | 1] | undefined>(
    undefined,
  );
  ...
  if (editorProvider !== undefined) {
    ...
    const CustomEditor = isObjectEditor
    ...
    editor = (
      <CustomEditor
        ...
        validatedSelection={
          isEditableGridCell(targetValue)
            ? targetValue.selectionRange
            : undefined
        }
