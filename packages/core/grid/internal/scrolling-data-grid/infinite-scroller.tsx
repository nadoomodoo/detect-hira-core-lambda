    ...
    React.useCallback(
      (e) => {
        if (e.touches.length === 0) {
          cbTimer.current = window.setTimeout(
            () => setHasTouches(false),
            delay,
          );
        }
...
export const InfiniteScroller: React.FC<Props> = (p) => {
  ...
  useKineticScroll(
    kineticScrollPerfHack && browserIsSafari.value,
    onScroll,
    scroller,
  );
  ...
  if (
    ...
    (lastProps.current?.height !== height || lastProps.current?.width !== width)
  ) {
      ...
      <ScrollRegionStyle isSafari={browserIsSafari.value}>
        ...
        >
          <div
            className={
              'dvn-scroll-inner' +
              (rightElement === undefined ? ' dvn-hidden' : '')
            }
          >
