...
- [코드≠금액·수량] 약품코드(9자리 표준코드 등)는 **식별자일 뿐** 금액·수량·단가가 아니다. 코드는 대개 그 행에서 가장 큰 숫자지만, **코드나 그 일부 자릿수를 금액/단가/수량 셀에 넣지 마라**. 각 숫자 셀은 그 컬럼(금액/단가/수량) 위치에 실제로 적힌 값만 읽는다.
...
- [코드≠금액] 약품코드 {CODE} 는 **식별자일 뿐 금액·수량·단가가 아니다**. 절대로 코드(또는 그 일부 자릿수)를 total_amount·unit_price·quantity 에 넣지 마라. 코드는 보통 그 행에서 가장 큰 숫자이지만 금액이 아니다.
- 각 숫자는 **해당 컬럼(단가/금액/수량 등) 아래 실제로 적힌 값**을 읽는다. "가장 큰 숫자"를 금액으로 고르지 말고, 금액 컬럼 위치의 값을 읽어라.
- 수량=1회/총 사용 개수(처방횟수·환자수 포함), 일수=투약일수, 총처방량=총사용량/총투여량/총소모량, 단가=1개 가격, 총금액=금액 컬럼의 값(대개 총처방량×단가).
- JSON 만 반환.`;
