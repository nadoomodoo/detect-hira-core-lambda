...
} from '../internal/data-grid-search/data-grid-search';
...
} from '../internal/data-grid/use-selection-behavior';
...
import { withAlpha } from '../internal/data-grid/color-parser';
import { combineRects, getClosestRect, pointInRect } from '../common/math';
import {
...
const DataGridOverlayEditor = React.lazy(
  async () =>
    await import(
      '../internal/data-grid-overlay-editor/data-grid-overlay-editor'
    ),
);
...
function shiftSelection(input: GridSelection, offset: number): GridSelection {
  if (
    input === undefined ||
    offset === 0 ||
    (input.columns.length === 0 && input.current === undefined)
  )
    return input;
...
export interface DataEditorProps
  extends Props,
    Pick<DataGridSearchProps, 'imageWindowLoader'> {
   ...
   * @group Advanced
   * */
  readonly attachmentEditorOverride?: ImageEditorType;
  /**
...
> = (p, forwardedRef) => {
  ...
  const drawFocusRing =
    drawFocusRingIn === 'no-editor' ? overlay === undefined : drawFocusRingIn;

  const rowMarkersObj =
    typeof p.rowMarkers === 'string' ? undefined : p.rowMarkers;

  const rowMarkers =
    rowMarkersObj?.kind ?? (p.rowMarkers as RowMarkerOptions['kind']) ?? 'none';
  const rowMarkerWidthRaw = rowMarkersObj?.width ?? p.rowMarkerWidth;
  const rowMarkerStartIndex =
    rowMarkersObj?.startIndex ?? p.rowMarkerStartIndex ?? 1;
  ...
  const maxColumnAutoWidth = Math.max(
    maxColumnAutoWidthIn ?? maxColumnWidth,
    minColumnWidth,
  );
  ...
  const remSize = React.useMemo(
    () => Number.parseFloat(docStyle.fontSize),
    [docStyle],
  );
  const {
    rowHeight,
  ...
  const rowMarkerWidth =
    rowMarkerWidthRaw ??
    (rowsIn > 10_000 ? 48 : rowsIn > 1000 ? 44 : rowsIn > 100 ? 36 : 32);
  ...
  const gridSelectionOuterMangled: GridSelection | undefined =
    React.useMemo((): GridSelection | undefined => {
  ...
  const abortControllerRef =
    React.useRef() as React.MutableRefObject<AbortController>;
  if (abortControllerRef.current === undefined)
    abortControllerRef.current = new AbortController();

  React.useEffect(() => () => abortControllerRef?.current.abort(), []);

  const [getCellsForSelection, getCellsForSeletionDirect] =
  ...
  const expectedExternalGridSelection = React.useRef<GridSelection | undefined>(
    gridSelectionOuter,
  );
  ...
  const drawHeader = whenDefined(
    ...
    React.useCallback<NonNullable<typeof drawHeaderIn>>(
      (args, draw) => {
        const res = drawHeaderIn?.(
          { ...args, columnIndex: args.columnIndex - rowMarkerOffset },
          draw,
        );
  ...
  const drawCell = whenDefined(
    ...
    React.useCallback<NonNullable<typeof drawCellIn>>(
      (args, draw) => {
        const res = drawCellIn?.(
          { ...args, col: args.col - rowMarkerOffset },
          draw,
        );
  ...
  const [setCurrent, setSelectedRows, setSelectedColumns] =
    useSelectionBehavior(
      gridSelection,
      setGridSelection,
  ...
  const rendererMap = React.useMemo(() => {
    ...
    const result: Partial<
      Record<
        InnerGridCellKind | GridCellKind,
  ...
  const getCellRenderer: <T extends InnerGridCell>(
    cell: T,
  ) => CellRenderer<T> | undefined = React.useCallback(
    <T extends InnerGridCell>(cell: T) => {
      if (!cell) return rendererMap[GridCellKind.Text] as unknown as CellRenderer<T>;
      if (cell.kind !== GridCellKind.Custom) {
        return rendererMap[cell.kind] as unknown as CellRenderer<T>;
      }
      return additionalRenderers?.find((x) =>
        x.isMatch(cell),
      ) as CellRenderer<T>;
    },
  ...
  let { sizedColumns: columns, nonGrowWidth } = useColumnSizer(
    ...
    clientSize[0] -
      (rowMarkerOffset === 0 ? 0 : rowMarkerWidth) -
      clientSize[2],
    minColumnWidth,
    maxColumnAutoWidth,
    mergedTheme,
  ...
  const totalHeaderHeight = enableGroups
  ...
  const mangledOnCellsEdited = React.useCallback<
    NonNullable<typeof onCellsEdited>
  >(
  ...
  const highlightFocus = drawFocusRing
  ...
  const getMangledCellContent = React.useCallback(
    ([col, row]: Item, forceStrict: boolean = false): InnerGridCell => {
      ...
      if (isRowMarkerCol) {
        ...
        const maybeFirstColumnHint = isFirst
        ...
        if (c?.trailingRowOptions?.disabled === true) {
          ...
          const icon =
            c?.trailingRowOptions?.addIcon ?? trailingRowOptions?.addIcon;
          return {
        ...
        if (forceStrict || experimental?.strict === true) {
          ...
          const isOutsideMainArea =
            ...
            row >= rowsRef.current;
          const isSelected =
            outerCol === vr.extras?.selected?.[0] &&
            row === vr.extras?.selected[1];
        ...
        if (rowMarkerOffset !== 0 && result?.span !== undefined) {
          result = {
            ...result, // FIXME: Mutate
            span: [
              result.span[0] + rowMarkerOffset,
              result.span[1] + rowMarkerOffset,
            ],
          };
        }
        return result;
  ...
  const mangledGetGroupDetails = React.useCallback<
    NonNullable<DataEditorProps['getGroupDetails']>
  >(
    (group) => {
      ...
      if (scrollRef.current !== null) {
        ...
        const trueRow =
          typeof row !== 'number'
            ...
            : row;
        const desiredX =
          typeof col !== 'number' && col.unit === 'px' ? col.amount : undefined;
        const desiredY =
          typeof row !== 'number' && row.unit === 'px' ? row.amount : undefined;
        if (grid !== null && canvas !== null) {
          ...
          if (trueCol !== undefined || trueRow !== undefined) {
            targetRect =
              grid.getBounds((trueCol ?? 0) + rowMarkerOffset, trueRow ?? 0) ??
              targetRect;
          ...
          if (targetRect !== undefined) {
            ...
            const freezeTrailingRowsEffective =
            ...
            let sLeft =
              frozenWidth * scale +
              scrollBounds.left +
              rowMarkerOffset * rowMarkerWidth * scale;
            ...
            if (
              dir === 'vertical' ||
              (typeof col === 'number' && col < freezeColumns)
            ) {
              scrollX = 0;
            } else if (
              dir === 'horizontal' ||
              (typeof row === 'number' &&
                row >= mangledRows - freezeTrailingRowsEffective)
            ) {
              scrollY = 0;
            }
  ...
  const getCustomNewRowTargetColumn = React.useCallback(
    (col: number): number | undefined => {
      const customTargetColumn =
        columns[col]?.trailingRowOptions?.targetColumn ??
      ...
      if (args.kind === 'cell') {
        ...
        if (col === 0 && hasRowMarkers) {
          ...
          if (onRowMoved !== undefined) {
            ...
            if (
              postClick === undefined ||
              postClick.checked === markerCell.checked
            )
              return;
          }
        ...
        if (hasRowMarkers && col === 0) {
          ...
          if (
            ...
            const newSlice: Slice = [
              Math.min(lastCol, col),
              Math.max(lastCol, col) + 1,
            ];
  ...
  const [scrollDir, setScrollDir] =
    React.useState<GridMouseEventArgs['scrollEdge']>();
    ...
    async (
      ...
      for (let x = 0; x < currentRange.width; x++) {
        for (let y = 0; y < currentRange.height; y++) {
          ...
          const patternCell =
            pattern[y % patternRange.height][x % patternRange.width];
  ...
  const onMouseUp = React.useCallback(
    (args: GridMouseEventArgs, isOutside: boolean) => {
      ...
      if (
        ...
        const newRange = {
          ...
          current: {
            ...gridSelection.current,
            range: combineRects(
              mouse.previousSelection.current.range,
              fillHighlightRegion,
            ),
          },
        };
      ...
      const [col, row] = args.location;
      const [lastMouseDownCol, lastMouseDownRow] =
        lastMouseSelectLocation.current ?? [];
      ...
      const handleMaybeClick = (a: GridMouseCellEventArgs): boolean => {
        const isValidClick =
          a.isTouch || (lastMouseDownCol === col && lastMouseDownRow === row);
        if (isValidClick) {
      ...
      if (args.isTouch) {
        ...
        if (args.isLongTouch === true) {
          if (
            args.kind === 'cell' &&
            itemsAreEqual(gridSelection.current?.cell, args.location)
          ) {
            ...
            args.kind === 'header' &&
            gridSelection.columns.hasIndex(col)
          ) {
            ...
            onGroupHeaderContextMenu?.(clickLocation, {
              ...args,
              preventDefault,
            });
            return;
  ...
  const onColumnMovedImpl = whenDefined(
    ...
    React.useCallback(
      (startIndex: number, endIndex: number) => {
        onColumnMoved?.(
          startIndex - rowMarkerOffset,
          endIndex - rowMarkerOffset,
        );
        if (columnSelect !== 'none') {
          setSelectedColumns(
            CompactSelection.fromSingleSelection(endIndex),
            undefined,
            true,
          );
        }
  ...
  const onItemHoveredImpl = React.useCallback(
    (args: GridMouseEventArgs) => {
      ...
      if (
        mouseDownData?.current?.button !== undefined &&
        mouseDownData.current.button >= 1
      )
        return;
      if (
        args.buttons !== 0 &&
        ...
        !mouseState.previousSelection.rows.hasIndex(
          mouseDownData.current.location[1],
        ) &&
        ...
        const end =
          Math.max(mouseDownData.current.location[1], args.location[1]) + 1;
        setSelectedRows(
      ...
      if (
        ...
        if (
          mouseState.fillHandle === true &&
          mouseState.previousSelection?.current !== undefined
        ) {
          ...
          const startedFromLastStickyRow =
            showTrailingBlankRow && selectedRow === rows;
          ...
          row =
            clampLimits === undefined
              ? row
              : clamp(row, clampLimits[0], clampLimits[1]);
  ...
  const onFinishEditing = React.useCallback(
    (
      ...
      if (
        overlay?.cell !== undefined &&
        newValue !== undefined &&
        isEditableGridCell(newValue)
      ) {
      ...
      if (gridSelection.current !== undefined && (movX !== 0 || movY !== 0)) {
        const isEditingTrailingRow =
          gridSelection.current.cell[1] === mangledRows - 1 &&
          newValue !== undefined;
        updateSelectedCell(
          clamp(
            gridSelection.current.cell[0] + movX,
            0,
            mangledCols.length - 1,
          ),
    ...
    (event: GridKeyEventArgs): boolean => {
      ...
      if (!overlayOpen && isHotkey(keys.clear, event, details)) {
        ...
        if (callbackResult !== false) {
          const toDelete =
            callbackResult === true ? gridSelection : callbackResult;
          ...
          for (const r of toDelete.rows?.toArray() ?? []) {
            deleteRange({
              x: rowMarkerOffset,
              y: r,
              width: columnsIn.length,
          ...
          for (const col of toDelete.columns?.toArray() ?? []) {
            deleteRange({
              x: col,
              y: 0,
              width: 1,
      ...
      if (isHotkey(keys.scrollToSelectedCell, event, details)) {
        ...
        columnSelect !== 'none' &&
        isHotkey(keys.selectColumn, event, details)
      ) {
        if (selectedColumns.hasIndex(col)) {
        ...
        rowSelect !== 'none' &&
        isHotkey(keys.selectRow, event, details)
      ) {
        ...
        !overlayOpen &&
        bounds !== undefined &&
        isHotkey(keys.activateCell, event, details)
      ) {
        ...
        gridSelection.current.range.height > 1 &&
        isHotkey(keys.downFill, event, details)
      ) {
        fillDown();
      } else if (
        gridSelection.current.range.width > 1 &&
        isHotkey(keys.rightFill, event, details)
      ) {
        fillRight();
      ...
      const mustRestrictRow =
        rowGroupingNavBehavior !== undefined &&
        rowGroupingNavBehavior !== 'normal';
  ...
  const onKeyDown = React.useCallback(
    (event: GridKeyEventArgs) => {
      ...
      if (
        ...
        gridSelection.current !== undefined &&
        event.key.length === 1 &&
        // 셀 선택 상태에서 타이핑 시 바로 편집 모드 진입 허용 문자:
        // [ -~] : 인쇄 가능한 ASCII (스페이스~틸데)
        // \u1100-\u11FF : 한글 자모 (초성/중성/종성)
        // \u3130-\u318F : 한글 호환 자모 (ㄱ~ㅎ, ㅏ~ㅣ)
        // \uAC00-\uD7AF : 한글 완성형 음절 (가~힣)
        /[ -~\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF]/g.test(event.key) &&
      ...
      function pasteToCell(
        ...
        if (
          ...
          if (r.kind === GridCellKind.Custom) {
            ...
            const newVal = (
              r as unknown as CustomRenderer<CustomCell<any>>
            ).onPaste?.(stringifiedRawValue, inner.data);
            ...
            return {
              location: target,
              value: newVal as EditableGridCell,
            };
          }
        }
        return undefined;
        ...
        do {
          if (onPaste === undefined) {
            ...
            const rawValue =
              text ??
              data
                ?.map((r) => r.map((cb) => cb.rawValue).join('\t'))
                .join('\t') ??
              '';
    ...
    async (e?: ClipboardEvent, ignoreFocus?: boolean) => {
      ...
      if (focused && getCellsForSelection !== undefined) {
        if (gridSelection.current !== undefined) {
          ...
          const toCopy = [...(selectedRows?.toArray() ?? [])];
          ...
          if (cells.some((x) => x instanceof Promise)) {
            ...
            copyToClipboardWithHeaders(
              ...
              range(columnsIn.length),
            );
          }
        } else if (selectedColumns.length > 0) {
          const results: (readonly (readonly GridCell[])[])[] = [];
          const cols: number[] = [];
          for (const col of selectedColumns?.toArray()) {
            let thunk = getCellsForSelection(
              {
                x: col,
                y: 0,
  ...
  const onCut = React.useCallback(
    async (e?: ClipboardEvent) => {
      ...
      if (gridSelection.current !== undefined) {
        ...
        effectiveSelection =
          onDeleteResult === true ? effectiveSelection : onDeleteResult;
  ...
  const selectionOutOfBounds =
    gridSelection.current !== undefined &&
    (gridSelection.current.cell[0] >= mangledCols.length ||
      gridSelection.current.cell[1] >= mangledRows);
  React.useLayoutEffect(() => {
  ...
  React.useImperativeHandle(
    forwardedRef,
    () => ({
      appendRow: (col: number, openOverlay?: boolean) =>
        appendRow(col + rowMarkerOffset, openOverlay),
  ...
  const [idealWidth, idealHeight] = React.useMemo(() => {
    ...
    const scrollbarWidth =
      experimental?.scrollbarWidthOverride ?? getScrollBarWidth();
    ...
    <ThemeContext.Provider value={mergedTheme}>
      ...
      >
        <DataGridSearch
          ...
          freezeTrailingRows={
            freezeTrailingRows +
            (showTrailingBlankRow && trailingRowOptions?.sticky === true
              ? 1
              : 0)
          }
        ...
        {overlay !== undefined && (
          <React.Suspense fallback={null}>
            <DataGridOverlayEditor
              ...
              getCellRenderer={getCellRenderer}
              className={
                experimental?.isSubGrid === true
                  ? 'click-outside-ignore'
                  : undefined
              }
              provideEditor={provideEditor}
              imageEditorOverride={imageEditorOverride}
              attachmentEditorOverride={attachmentEditorOverride}
