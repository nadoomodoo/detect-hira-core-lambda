...
const NumberOverlayEditor = React.lazy(
  async () =>
    await import(
      '../internal/data-grid-overlay-editor/private/number-overlay-editor'
    ),
);
...
export const numberCellRenderer: InternalCellRenderer<NumberCell> = {
  ...
  measure: (ctx, cell, theme) =>
    ctx.measureText(cell.displayData).width + theme.cellHorizontalPadding * 2,
  onDelete: (c) => ({
    ...c,
    data: undefined,
  }),
  ...
  onPaste: (toPaste, cell, details) => {
    const newNumber =
      typeof details.rawValue === 'number'
        ? details.rawValue
        : Number.parseFloat(
            typeof details.rawValue === 'string' ? details.rawValue : toPaste,
          );
