...
export interface DataGridSearchProps
  extends Omit<ScrollingDataGridProps, 'prelightCells'> {
...
const DataGridSearch: React.FunctionComponent<DataGridSearchProps> = (p) => {
  ...
  const [searchID] = React.useState(
    () => 'search-box-' + Math.round(Math.random() * 1000),
  );
  ...
  const beginSearch = React.useCallback(
    (str: string) => {
      ...
      const tick = async () => {
        ...
        let added = false;
        let row = 0;
        for (row = 0; row < startY; row++) {
          const d = data[row];
          let col = 0;
          for (col = 0; col < d.length; col++) {
            const cell = d[col];
            ...
            switch (cell.kind) {
              ...
              case GridCellKind.Boolean:
                testString =
                  typeof cell.data === 'boolean'
                    ? cell.data.toString()
                    : undefined;
                break;
              case GridCellKind.Image:
              case GridCellKind.Attachment:
